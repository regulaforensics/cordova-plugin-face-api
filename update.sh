...
#   if beta, core module name will be changed to @regulaforensics/cordova-plugin-document-reader-core-fullrfid-beta in example/package.json)
...
if [ "$Branch_ios" == 'Beta' ]; then
	...
	if [ "$Branch_ios" == 'Release' ]; then
        ...
        rm -rf "$Base_path/../../cordova-plugin-document-reader-api"
		exit 1
	fi
fi
...
if [ "$Core_module_type" == 'Beta' ]; then
	...
	if [ "$Core_module_type" == 'Release' ]; then
		...
		echo "Incorrect core module type(must be Beta or Release)"
		rm -rf "$Base_path/../../cordova-plugin-document-reader-api"
	    exit 1
	fi
fi

PKG_NAME="@regulaforensics/cordova-plugin-document-reader-api$DEST_TYPE"
CORE_PKG_NAME="@regulaforensics/cordova-plugin-document-reader-core-fullrfid$CORE_TYPE"
PKG_NAME_CLEAN="cordova-plugin-document-reader-api$DEST_TYPE"
CORE_PKG_NAME_CLEAN="cordova-plugin-document-reader-core-fullrfid$CORE_TYPE"
...
cd "$Base_path"
cd "../"
git clone --branch "$GITHUB_BRANCH" https://github.com/regulaforensics/DocumentReader-Cordova-Plugin.git cordova_temp_dir
